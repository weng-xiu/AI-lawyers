    ...
    for (const answer of possibleAnswers) {
      ...
      try {
        ...
        if (loginResponse.data.code === 200) {
          ...
          console.log('用户类型:', userInfoResponse.data.user.userType);
          return; // 登录成功，退出循环
        } else {
          console.log(`验证码 ${answer} 登录失败:`, loginResponse.data.msg);
        }
      } catch (error) {
        console.log(`验证码 ${answer} 登录出错:`, error.response ? error.response.data.msg : error.message);
      }
    }
    
    console.log('所有验证码答案都尝试失败');
  } catch (error) {
